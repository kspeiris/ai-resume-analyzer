...
export const exportAnalysisReport = async (analysis) => {
  try {
    ...
    doc.setFontSize(48);
    doc.setTextColor(
      analysis.scores.overall >= 80 ? 16 : 
      analysis.scores.overall >= 60 ? 245 : 239,
      analysis.scores.overall >= 80 ? 185 : 
      analysis.scores.overall >= 60 ? 158 : 68,
      analysis.scores.overall >= 80 ? 129 : 
      analysis.scores.overall >= 60 ? 11 : 68
    );
    ...
    const scoreData = [
      ...
      ['Keyword Match', `${analysis.scores.keyword}%`, 
       analysis.scores.keyword >= 70 ? 'Good' : 'Needs Improvement'],
      ['Semantic Similarity', `${analysis.scores.semantic}%`,
       analysis.scores.semantic >= 70 ? 'Good' : 'Needs Improvement'],
      ['Formatting', `${analysis.scores.format}%`,
